# To do:
# Aligned sequence is not printing properly
...
def find_best_align(s1, s2, l1, l2):
    """Finds the alignment of any sequences that give the best match (highest score)"""
    ...
    for i in range(l1):
        ...
        if z > my_best_score:
            ...
            my_best_align.append("." * i + s2)  # prints number of '.' to get to startpoint (which is i here)
...
def output_file(a, b, my_best_align, my_best_score, s1):
    ...
    g.write(f'My best score is {str(my_best_score)}\n'
            f'{len(my_best_align)} alignments are presented with the same score.\n')
    
    for i in range(len(my_best_align)):
        g.write(f"{my_best_align[i]}\n{s1}\n\n")
